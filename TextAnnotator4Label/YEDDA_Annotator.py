...
class Example(Frame):
    def __init__(self, parent):
        ...
        if len(self.pressCommand) > 20:
            ...
            self.textRow = 20
        self.textColumn = 5
        self.tagScheme = "BI"   # we choose BI depends on training system "BMES"
        ...
        try:
            ...
            if matched_span[1] > 0:
                ...
                if command == "q":
                    ...
                    old_key = list(self.pressCommand.keys())[list(self.pressCommand.values()).index(old_entity_type)]
    ...
    def deleteTextInput(self, event):
        ...
        if len(get_input) > 0:
            followHalf_content = followHalf_content.replace(get_input, ''.encode('utf-8'), 1)
        content = aboveHalf_content + followHalf_content
    ...
    def savenewPressCommand(self):
        ...
        self.configFile = tkinter.filedialog.asksaveasfilename(initialdir="./configs/",
        ...
        with open(self.configFile, 'wb') as fp:
            pickle.dump(self.pressCommand, fp)
        self.setMapShow()
        tkinter.messagebox.showinfo("Save New Map Notification",
    ...
    def generateSequenceFile(self):
        if (".ann" not in self.fileName) and (".txt" not in self.fileName):
            ...
            return -1
        fileLines = open(self.fileName, 'rU', encoding='utf-8').readlines()
